...
vector<double> affarclen(const vector<double> &x, const vector<double> &y) {
    vector<double> dx, dy, ddx, ddy, affal;
	double AL=0.0;
	dx.push_back(0.0);
	dy.push_back(0.0);
	for (int i=1; i<n; i++) {
	...
	ddy.push_back(0.0);
	for (int i=1; i<n; i++) {
		ddx.push_back(dx[i]-dx[i-1]);
		ddy.push_back(dy[i]-dy[i-1]);
	}
	affal.push_back(0.0);
	for (int i=1; i<n; i++) {
		AL += pow(dx[i]*ddy[i] - dy[i]*ddx[i] ,1/3);
		affal.push_back(AL);
	}
  ...
  if (argc != 6) {
    ...
    */
	...
	if (X.size() == 1) {
	  ...
	  double cur_alpha = (randomDouble() - 0.5) * 2 * alpha;
	  
	  //	  cout << cur_alpha << endl;

	  rotate(X,Y,cur_alpha);
	  ...
	  */
	    ...
	    */
	    ...
	    vector<double> eradquick = eradius_from_exy(exy,GammaRad);

	    e1quick = einvar_from_exy(exy,GammaI1);
